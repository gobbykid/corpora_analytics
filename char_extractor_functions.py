...
with open('assets/CSV non-characters/nationalities.csv') as f:
    reader = csv.reader(f)
    nationalities = [row[0].lower() for row in reader]

with open('assets/CSV non-characters/countries.csv') as f:
    reader = csv.reader(f)
    countries = [row[0].lower() for row in reader]

with open('assets/CSV non-characters/UK_counties.csv') as f:
    reader = csv.reader(f)
    uk_counties = [row[0].lower() for row in reader]

with open('assets/CSV non-characters/UK_cities.csv') as f:
    reader = csv.reader(f)
    uk_cities = [row[0].lower() for row in reader]

with open('assets/CSV non-characters/religious_words.csv') as f:
    reader = csv.reader(f)
    religious_words = [row[0].lower() for row in reader]

with open('assets/CSV non-characters/common_geographical_names.csv') as f:
    reader = csv.reader(f)
    geo_names = [row[0].lower() for row in reader]

with open('assets/CSV non-characters/exclamations.csv') as f:
    reader = csv.reader(f)
...
def get_proper_nouns(book):
    proper_nouns = {}
    list_of_sentences = syntok_sentence_tokenizer(book) #we use syntok's segmenter in order to avoid the splitting of words like "Mr." or "Mrs." into "Mr" and "." (which would be the case if we used nltk's sent_tokenize)
